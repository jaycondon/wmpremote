...
public class WMP extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// This is to get the height of the screen so to stretch the listView to the bottom
	...
	// Got this method from here to close the database when the activity finishes
	...
	public void pop(String weekId) {
		...
		// it easier in the future to add Create a few weekly different meal plans
		Cursor dayC = db.getMealPlan(weekId);
		Log.d("After getting cursor", "After getting cursor");
		
		// Custom display adapter got for http://stackoverflow.com/questions/5300787/how-do-i-create-a-custom-cursor-adapter-for-a-listview-for-use-with-images-and-t
		// and http://www.gustekdev.com/2013/05/custom-cursoradapter-and-why-not-use.html
		disAdapter = new DisplayCursorAdapter(this, dayC, false);

		// Got the list idea from http://www.vogella.com/tutorials/AndroidListView/article.html
		eachDay = (ListView) findViewById(R.id.list);
		
		// attach the created adpter to the list
		eachDay.setAdapter(disAdapter);
		
		// got context menu idea from http://developer.android.com/guide/topics/ui/menus.html
	...
	@Override // got context menu idea from http://developer.android.com/guide/topics/ui/menus.html
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.add_Recipe:
			...
			// got this from http://developer.android.com/training/basics/intents/result.html
	...
	// This is used to get the result back from DisplayRecipe.class using the intent started above
	// got this idea from 
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1) {
			...
			if (resultCode == RESULT_OK) {
				...
				// Adds the recipe to meal plan using the dayselected got from adapter class when
				// a view is selected MealTimeSelected is got when touching a particular view inside
				// each list item and the recipeid is got from the returned id from DisplayRecipe
			...
			if (resultCode == RESULT_CANCELED) {
				...
				Toast.makeText(getBaseContext(), "An error has occured", Toast.LENGTH_SHORT).show();
			}
		}
	}// onActivityResult
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.wm, menu);
		return true;
	}
	...
	public static class PlaceholderFragment extends Fragment {
		...
		public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			View rootView = inflater.inflate(R.layout.fragment_wm, container,
					false);
			return rootView;
		}
	}
